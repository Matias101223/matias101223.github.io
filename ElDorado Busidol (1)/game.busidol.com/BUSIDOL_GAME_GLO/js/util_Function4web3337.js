...
util4web.screenAdjust = function()
{
	...
	if (util4web.is_mobile)
	{
		...
		// console.log("  gamebox_w:" + gamebox_w);
		// console.log("  gamebox_h:" + gamebox_h);

		// $('#AD_FIT_BG').attr({ width: device_w, height: device_h });
		var gamebox_top = 0; // 20240718_dblee 50 -> 0 수정
		var mobile_w = 0; // 20240718_dblee 50 -> 0 수정
		// var mobile_w = parseInt($('#AD_BOX_MOBILE .kakao_ad_area').css('width'));

		// 20240722_dblee_1652 화면 사이즈 오류 수정함.
		if (device_w < 1280) {
			var wScale = device_w / 1280;
			var hScale = (device_h - gamebox_top) / 720;
		}
		else {
			var wScale = 1280 / device_w;
			var hScale = 720 / (device_h - gamebox_top);
		}
		...
		if (app_name == 'JUMP') {
			wScale = device_w / 720;
			// gamebox_h = (gamebox_w * 720 / 1280);
			hScale = (device_h - gamebox_top) / 1280;
			gamebox_h =(device_h - 0);
			mobile_top = (device_h - gamebox_h) / 2;
		...
		if (busidol_ip) {
			// alert(" device_w:"+device_w+" device_h:"+device_h);
			// console.log(" wScale:"+wScale+" hScale:"+hScale);
			// console.log(" device_w:"+device_w+" device_h:"+device_h);
			// console.log('MOBILE width:'+ $('#AD_FIT_BG').attr('width'));
		}
	}
	else
	{
		...
		if (busidol_ip) {
			// console.log('PC width:'+ $('#AD_FIT_BG').attr('width'));
		} else {
			$('#AD_FIT_BG').attr({ width: device_w, height: device_h });
		}


// 20240718_dblee 광고 제거로 주석처리함
/*
...
*/
		...
		if (app_name == 'JUMP') {
			// 세로 모드이기때문에 hScale 값을 구하고 비율을 같이 사용함.
			var hScale = (device_h - 100) / 1280;
			gamebox_h = hScale * 1280;
			gamebox_top = (device_h - gamebox_h) / 2;

			var wScale = hScale;
			// var wScale = gamebox_w / 1280;
			// var gamebox_h = gamebox_w * 720 / 1280;

			var gamebox_left = (device_w - gamebox_w) / 2;



			if (gamebox_top < 0) gamebox_top = 0;

		} else {
			// var gamebox_left = 0;
			var gamebox_left = (device_w - gamebox_w) /2; // 20240718_dblee 광고 제거로 위치 조정함.
		}
	...
	if (debug ==  1)
	{
		command = 'scale(' + 1 + ',' + hScale + ')';
	}

	// console.log("  wScale:" + wScale + "   hScale:" + hScale + "   gamebox_w:" + gamebox_w + "   gamebox_h:" + gamebox_h);
	// console.log("  device_w:" + device_w + "   device_h:" + device_h );
	// if (util4web.is_mobile && busidol_ip) {	// 디버깅용
	if (util4web.is_mobile) {
		// 20210628_dblee 특정 모바일 또는 브라우저에서 scale 조정해도 화면 밖으로 넘치는 현상을 수정함.
		// hScale > 1.2 조건문은 정상적인 브라우저에서는 1.2 값을 넘지 않는 것으로 보여 이 수치 값을 사용함
		if (hScale > 1.2) {

			// command = 'scale(' + 0.97 + ',' + hScale + ')';

			$('#div_body_app').css({
				"left": gamebox_left + 'px',
				"top": gamebox_top + 'px',   // 20210129 -카카오 광고 TV를 상단에 배치 할려면 주석처리해야 한다.
				'width': '100%',
				'transform': command,
				'-ms-transform': command, /* IE 9 */
				'-webkit-transform': command, /* Safari and Chrome */
